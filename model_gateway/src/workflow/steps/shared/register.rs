...
mod tests {
    ...
    fn worker(model_id: &str, alias: &str) -> Arc<dyn Worker> {
        Arc::new(
            BasicWorkerBuilder::new("http://worker:8080")
                .model(ModelCard::new(model_id).with_alias(alias))
                .build(),
        )
    }

    #[test]
    fn create_only_rejects_duplicate_without_replacing_model_aliases() {
        let registry = WorkerRegistry::new();
        let original_id = registry
            .register(worker("original-model", "original-alias"))
            .unwrap();

        let result = register_worker(
            &registry,
            worker("replacement-model", "replacement-alias"),
            WorkerRegistrationMode::CreateOnly,
        );
        ...
        assert_eq!(registry.get_by_model("original-alias").len(), 1);
        assert!(registry.get_by_model("replacement-model").is_empty());
        assert!(registry.get_by_model("replacement-alias").is_empty());
    }
}
