...
/// Resources whose lifetime is exactly one prefill phase. Only constructed
/// when prefill admission is enabled; with admission disabled, prefill load
/// follows the legacy full-request lifetime instead.
///
/// The permit is held exclusively for a single PD request (`new`) or shared
/// across the sub-requests of one batched request (`new_shared`).
pub struct PrefillPhaseGuard {
    _load: WorkerLoadGuard,
    _permit: Option<PrefillPermit>,
    _shared_permit: Option<Arc<PrefillPermit>>,
}
...
mod tests {
    ...
    async fn rejects_when_queue_is_full() {
        ...
        #[expect(
            ...
            reason = "test waiter task; completes or is aborted before the test ends"
        )]
    ...
    async fn phase_guard_releases_worker_load_and_admission_slot_together() {
        ...
        let gate = PrefillAdmission::new(1, 0, Duration::from_secs(1));
        let permit = gate.admit().await.unwrap();
        let guard = PrefillPhaseGuard::new(worker.clone(), None, permit);
    ...
    #[tokio::test]
    async fn shared_batch_phase_guard_releases_after_last_prefill_finishes() {
        let worker: Arc<dyn Worker> = Arc::new(
            BasicWorkerBuilder::new("http://prefill")
                .worker_type(WorkerType::Prefill)
                .build(),
        ...
        let gate = PrefillAdmission::new(1, 0, Duration::from_secs(1));
        let permit = Arc::new(gate.admit().await.unwrap());
        let first = PrefillPhaseGuard::new_shared(worker.clone(), None, permit.clone());
        let second = PrefillPhaseGuard::new_shared(worker.clone(), None, permit.clone());
        let third = PrefillPhaseGuard::new_shared(worker.clone(), None, permit);
